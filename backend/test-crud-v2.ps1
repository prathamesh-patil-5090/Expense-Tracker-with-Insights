# Expense CRUD API Test Script - PowerShell
...
try {
    ...
    Write-Failure "Authentication failed: $_"
    exit 1
}
...
try {
    [string]$categoriesUrl = $BaseUrl + "/categories?userId=" + $userId
    ...
    Write-Failure "Failed to get categories: $_"
    exit 1
}
...
try {
    [string]$getUrl = $BaseUrl + "/expenses/" + $expenseId
    ...
    Write-Failure "READ single expense failed: $_"
    $failedTests++
}
...
try {
    [string]$listUrl = $BaseUrl + "/expenses?userId=" + $userId + "&limit=5&offset=0"
    ...
    Write-Failure "READ list failed: $_"
    $failedTests++
}
...
try {
    [string]$statsUrl = $BaseUrl + "/expenses/stats/" + $userId
    ...
    Write-Failure "READ statistics failed: $_"
    $failedTests++
}
...
try {
    [string]$updateUrl = $BaseUrl + "/expenses/" + $expenseId
...
try {
    [string]$deleteUrl = $BaseUrl + "/expenses/" + $expenseId
...
try {
    [string]$verifyUrl = $BaseUrl + "/expenses/" + $expenseId
    $verifyResponse = Invoke-RestMethod -Uri $verifyUrl `
        -Method GET -Headers $headers -ErrorAction Stop
    
    Write-Failure "Expense still exists after deletion"
    $failedTests++
    ...
    if ($_.Exception.Response.StatusCode -eq 404) {
        ...
        Write-Failure "Unexpected error verifying deletion: $_"
        $failedTests++
    }
}
...
try {
    ...
    Write-Failure "Should reject missing amount"
    $failedTests++
...
try {
    ...
    Write-Failure "Should reject zero amount"
    $failedTests++
...
try {
    ...
    Write-Failure "Should reject invalid category"
    $failedTests++
