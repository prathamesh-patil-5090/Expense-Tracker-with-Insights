...
try {
    $categoriesResponse = Invoke-RestMethod -Uri "$BaseUrl/categories?userId=$userId" `
...
try {
    $getResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses/$expenseId" -Method GET `
...
try {
    $listResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses?userId=$userId&limit=5&offset=0" `
        -Method GET -Headers $headers -ErrorAction Stop
...
try {
    $statsResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses/stats/$userId" `
        -Method GET -Headers $headers -ErrorAction Stop
...
try {
    $updateResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses/$expenseId" -Method PUT `
...
try {
    $deleteResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses/$expenseId" -Method DELETE `
        -Headers $headers -ErrorAction Stop
...
try {
    $verifyResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses/$expenseId" `
        -Method GET -Headers $headers -ErrorAction Stop
...
try {
    $invalidResponse = Invoke-RestMethod -Uri "$BaseUrl/expenses" -Method POST `
